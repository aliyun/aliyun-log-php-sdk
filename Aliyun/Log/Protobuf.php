...
class Protobuf
{
     ...
     */
    public static function read_varint($fp, &$limit = \null)
    {
        $value = '';
        $len = 0;
        do {
            // Keep reading until we find the last byte
            $b = \fread($fp, 1);
            if ($b === \false) {
    ...
    public static function skip_field($fp, $wire_type)
    {
        switch ($wire_type) {
            case 0:
                // varint
                return \Aliyun\Log\Protobuf::skip_varint($fp);
            case 1:
                // 64bit
                if (\fseek($fp, 8, \SEEK_CUR) === -1) {
                    throw new \Exception('skip(' . \Aliyun\Log\Protobuf::get_wiretype(1) . '): Error seeking');
                }
                return 8;
            case 2:
                // length delimited
                $varlen = 0;
                $len = \Aliyun\Log\Protobuf::read_varint($fp, $varlen);
                if (\fseek($fp, $len, \SEEK_CUR) === -1) {
                    throw new \Exception('skip(' . \Aliyun\Log\Protobuf::get_wiretype(2) . '): Error seeking');
                }
                return $len - $varlen;
            ...
            case 5:
                ...
                if (\fseek($fp, 4, \SEEK_CUR) === -1) {
                    throw new \Exception('skip(' . \Aliyun\Log\Protobuf::get_wiretype(5) . '): Error seeking');
                }
                return 4;
            default:
                throw new \Exception('skip(' . \Aliyun\Log\Protobuf::get_wiretype($wire_type) . '): Unsupported wire_type');
        }
    }
    /**
     * Read a unknown field from the stream and return its raw bytes
     */
    public static function read_field($fp, $wire_type, &$limit = \null)
    {
        switch ($wire_type) {
            case 0:
                // varint
                return \Aliyun\Log\Protobuf::read_varint($fp, $limit);
            case 1:
                // 64bit
                $limit -= 8;
                return \fread($fp, 8);
            ...
            case 5:
                ...
                return \fread($fp, 4);
            default:
                throw new \Exception('read_unknown(' . \Aliyun\Log\Protobuf::get_wiretype($wire_type) . '): Unsupported wire_type');
        }
    }
    /**
    ...
    public static function toString($key, $value)
    {
        ...
        if (\is_array($value)) {
            ...
            if (\is_object($value)) {
                ...
                $safevalue = \addcslashes($value, "\x00..\x1f..\xff");
