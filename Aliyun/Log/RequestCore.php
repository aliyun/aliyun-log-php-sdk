...
class RequestCore
{
     ...
     * @return $this A reference to the current instance.
     */
    public function set_read_stream($resource, $size = \null)
    {
    ...
    public function set_proxy($proxy)
    {
        ...
        $proxy['user'] = isset($proxy['user']) ? $proxy['user'] : \null;
        $proxy['pass'] = isset($proxy['pass']) ? $proxy['pass'] : \null;
        $proxy['port'] = isset($proxy['port']) ? $proxy['port'] : \null;
        $this->proxy = $proxy;
        return $this;
    }
    /**
    ...
    {
        $this->seek_position = isset($position) ? (int) $position : \null;
        return $this;
    }
    /**
    ...
    public function streaming_read_callback($curl_handle, $file_handle, $length)
    {
        ...
        if ($this->read_stream_read == 0 && isset($this->seek_position) && $this->seek_position !== \ftell($this->read_stream)) {
            if (\fseek($this->read_stream, $this->seek_position) !== 0) {
                throw new \Aliyun\Log\RequestCoreException('The stream does not support seeking and is either not at the requested position or the position is unknown.');
            }
        }
    ...
    public function prep_request()
    {
        ...
        \curl_setopt($curl_handle, \CURLOPT_FILETIME, \true);
        \curl_setopt($curl_handle, \CURLOPT_FRESH_CONNECT, \false);
        \curl_setopt($curl_handle, \CURLOPT_MAXREDIRS, 5);
        \curl_setopt($curl_handle, \CURLOPT_HEADER, \true);
        \curl_setopt($curl_handle, \CURLOPT_RETURNTRANSFER, \true);
        \curl_setopt($curl_handle, \CURLOPT_TIMEOUT, 50);
        \curl_setopt($curl_handle, \CURLOPT_CONNECTTIMEOUT, 50);
        \curl_setopt($curl_handle, \CURLOPT_NOSIGNAL, \true);
        \curl_setopt($curl_handle, \CURLOPT_REFERER, $this->request_url);
        \curl_setopt($curl_handle, \CURLOPT_USERAGENT, $this->useragent);
        \curl_setopt($curl_handle, \CURLOPT_READFUNCTION, array($this, 'streaming_read_callback'));
        // Verification of the SSL cert
        if ($this->ssl_verification) {
            \curl_setopt($curl_handle, \CURLOPT_SSL_VERIFYPEER, \true);
        ...
        // Debug mode
        if ($this->debug_mode) {
            \curl_setopt($curl_handle, \CURLOPT_VERBOSE, \true);
        }
        // Handle open_basedir & safe mode
        if (!\ini_get('safe_mode') && !\ini_get('open_basedir')) {
            \curl_setopt($curl_handle, \CURLOPT_FOLLOWLOCATION, \true);
        }
        // Enable a proxy connection if requested.
        if ($this->proxy) {
            \curl_setopt($curl_handle, \CURLOPT_HTTPPROXYTUNNEL, \true);
            ...
            case self::HTTP_PUT:
                ...
                if (isset($this->read_stream)) {
                    if (!isset($this->read_stream_size) || $this->read_stream_size < 0) {
                        throw new \Aliyun\Log\RequestCoreException('The stream size for the streaming upload cannot be determined.');
                    }
                    \curl_setopt($curl_handle, \CURLOPT_INFILESIZE, $this->read_stream_size);
                    \curl_setopt($curl_handle, \CURLOPT_UPLOAD, \true);
                } else {
                    \curl_setopt($curl_handle, \CURLOPT_POSTFIELDS, $this->request_body);
                }
                break;
            case self::HTTP_POST:
                \curl_setopt($curl_handle, \CURLOPT_POST, \true);
                \curl_setopt($curl_handle, \CURLOPT_POSTFIELDS, $this->request_body);
                break;
            case self::HTTP_HEAD:
     ...
     * @return \Aliyun\Log\ResponseCore A <ResponseCore> object containing a parsed HTTP response.
     */
    public function process_response($curl_handle = \null, $response = \null)
    {
        if ($response) {
    ...
    public function send_request($parse = \false)
    {
        ...
        if ($this->response === \false) {
            throw new \Aliyun\Log\RequestCoreException('cURL resource: ' . (string) $curl_handle . '; cURL error: ' . \curl_error($curl_handle) . ' (' . \curl_errno($curl_handle) . ')');
        }
     ...
     */
    public function send_multi_request($handles, $opt = \null)
    {
        \set_time_limit(0);
        ...
        do {
            ...
            while ($done = \curl_multi_info_read($multi_handle)) {
                ...
                if ($done['result'] > 0) {
                    throw new \Aliyun\Log\RequestCoreException('cURL resource: ' . (string) $done['handle'] . '; cURL error: ' . \curl_error($done['handle']) . ' (' . $done['result'] . ')');
            ...
            foreach ($to_process as $pkey => $done) {
                ...
                $key = \array_search($done['handle'], $handle_list, \true);
